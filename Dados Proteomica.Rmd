...
E podemos plotar para cada indivíduo:
...
```{r, fig.cap="Gráfico de esferas", fig.align='center'}
...
```{r, fig.cap="Gráfico de elipses", fig.align='center'}
corrplot(x, method="ellipse")
```

```{r, fig.cap="Gráfico de niveis", fig.align='center'}
require(lattice)
levelplot(x)
```

```{r, fig.cap="Ggplot2", fig.align='center'}
library(ggplot2)
library(reshape)
x.m = melt(x)
ggplot(x.m, aes(X1, X2, fill = value)) + geom_tile() + 
  scale_fill_gradient(low = "blue",  high = "yellow")
```

Podemos calcular também as correlações par a par e graficá-las.
